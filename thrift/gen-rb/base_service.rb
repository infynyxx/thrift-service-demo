    ...
    module Base
      module BaseService
        class Client
          ...
          def send_getStaus()
            send_message('getStaus', GetStaus_args)
          end

          def recv_getStaus()
            result = receive_message(GetStaus_result)
            return result.success unless result.success.nil?
            raise ::Thrift::ApplicationException.new(::Thrift::ApplicationException::MISSING_RESULT, 'getStaus failed: unknown result')
          end
        ...
        class Processor
          ...
          def process_getStaus(seqid, iprot, oprot)
            args = read_args(iprot, GetStaus_args)
            result = GetStaus_result.new()
            result.success = @handler.getStaus()
            write_result(result, oprot, 'getStaus', seqid)
          end
        ...
        class GetStaus_result
          include ::Thrift::Struct, ::Thrift::Struct_Union
          SUCCESS = 0

          FIELDS = {
            SUCCESS => {:type => ::Thrift::Types::I32, :name => 'success', :enum_class => Base::ServiceStatus}
          }

          def struct_fields; FIELDS; end

          def validate
            unless @success.nil? || Base::ServiceStatus::VALID_VALUES.include?(@success)
